#include"LSVKRenderPass.h"
#include"LSVKDevice.h"
#include"Graphic/LsVKFramebuffer.h"

namespace lse {
	LSVKRenderPass::LSVKRenderPass(LSVKDevice *device, const std::vector<Attachment>& attachments, const std::vector<RenderSubPass>& subPasses )
	:mDevice(device),mAttachments(attachments),mSubPasses(subPasses){
		//1.若attachments和renderSubPass为空，则默认构造一个
		if(mSubPasses.empty()) {
			mAttachments = {{
	...
	LSVKRenderPass::~LSVKRenderPass() {
		VK_D(RenderPass,mDevice->GetHandle(),mHandle);
	}

	void LSVKRenderPass::BeginRenderPass(VkCommandBuffer cmdBuffer, LSVKFramebuffer *frameBuffer, const std::vector<VkClearValue> &clearValues) const {
		VkRect2D renderArea = {};
	...
	void LSVKRenderPass::EndRenderPass(VkCommandBuffer cmdBuffer) const {
