...
namespace lse{
    ...
    static const char* LSKeyToStr(Key keyCode){
        switch (keyCode) {
    ...
    static const char* LSKeyModToStr(KeyMod keyMod){
        switch (keyMod) {
            ENUM_TO_STR(LS_MOD_SHIFT);
            ENUM_TO_STR(LS_MOD_CONTROL);
            ENUM_TO_STR(LS_MOD_ALT);
            ENUM_TO_STR(LS_MOD_SUPER);
            ENUM_TO_STR(LS_MOD_CAPS_LOCK);
            ENUM_TO_STR(LS_MOD_NUM_LOCK);
        }
        return "unknown";
    }

    class LSKeyPressEvent : public LSEvent{
    public:
        LSKeyPressEvent(Key keycode, KeyMod ketMod,bool repeat) : mKeyCode(keycode), mKeyMod(ketMod), mRepeat(repeat){
        ...
        [[nodiscard]] std::string ToString() const override{
            std::stringstream ss;
            ss<<LSEvent::ToString();
            ss<<"( keyCode=" <<LSKeyToStr(mKeyCode);
        ...
        [[nodiscard]] bool IsCapsLockPressed() const { return mKeyMod & LS_MOD_CAPS_LOCK; };
        [[nodiscard]] bool IsNumLockPressed() const { return mKeyMod & LS_MOD_NUM_LOCK; };
    ...
    class LSKeyReleaseEvent : public LSEvent{
    public:
        LSKeyReleaseEvent(Key keyCode) : mKeyCode(keyCode){
